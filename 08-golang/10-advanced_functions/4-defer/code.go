...
func logAndDelete(users map[string]user, name string) (log string) {
	...
	// Defer the deletion of the user from the map.
	// This means `delete(users, name)` will be executed after
	// the surrounding function (`logAndDelete`) finishes executing,
	// regardless of which return path is taken (early returns included).
